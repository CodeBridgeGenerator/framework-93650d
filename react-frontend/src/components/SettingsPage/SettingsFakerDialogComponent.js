...
import settingsFakerFactory from "./settingsFakerFactory";
...
const SettingsFakerDialogComponent = (props) => {
...
const [domainIds, setdomainIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("environments")
                        ...
                        .then((data) => {
                        setsettingsIds(data.data);
                        });
client.service("environments")
                        .find({ query: { $select: ["_id","type"] } })
                        .then((data) => {
                        setdomainIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = settingsFakerFactory(props.user,fakerCount,settingsIds,domainIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("settings").create(elem));
...
export default connect(mapState, mapDispatch)(SettingsFakerDialogComponent);

...
import seederData from "./codeEditor.seeder.json";
...
const CodeEditorSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("codeEditor").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(CodeEditorSeederDialogComponent);

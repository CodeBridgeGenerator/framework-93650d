...
destFolder: _entity?.destFolder,
destFilename: _entity?.destFilename,
customrelationship: _entity?.customrelationship,
        };

        setLoading(true);
        try {
            
        const result = await client.service("servicegentemp").patch(_entity._id, _data);
        props.onHide();
    ...
    return (
        <Dialog header="Edit Service Gen Template" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>

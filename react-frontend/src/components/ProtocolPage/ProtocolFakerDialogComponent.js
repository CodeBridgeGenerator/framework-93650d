...
import protocolFakerFactory from "./protocolFakerFactory";
...
const ProtocolFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("settings")
                        .find({ query: { $select: ["_id","domain"] } })
                        .then((data) => {
                        setprotocolIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = protocolFakerFactory(props.user,fakerCount,protocolIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("protocol").create(elem));
...
export default connect(mapState, mapDispatch)(ProtocolFakerDialogComponent);

...
const SingleEnvironmentsPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((res) => {
                set_entity(res || {});
                const env = Array.isArray(res.env)
            ? res.env.map((elem) => ({ _id: elem._id, appName: elem.appName }))
            : res.env
                ? [{ _id: res.env._id, appName: res.env.appName }]
                : [];
        setEnv(env);
            })
            .catch((error) => {
    ...
    return (
        ...
        <div className="col-12 flex flex-column align-items-center">
            ...
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm">Env</label>
                    {env.map((elem) => (
                        <Link key={elem._id} to={`/framework/${elem._id}`}>
                            <div className="card">

...
change: _entity?.change,
        };

        setLoading(true);
        try {
            
        const result = await client.service("codechange").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info codechange updated successfully" });
    ...
    return (
        <Dialog header="Edit Codechange" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="codechange-edit-dialog-component">

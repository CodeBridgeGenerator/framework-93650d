...
const CodeEditorDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.dbService}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.name}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "codeEditor"});
...
<Column field="dbService" header="DbService" body={pTemplate0} filter={selectedFilterFields.includes("dbService")} hidden={selectedHideFields?.includes("dbService")}  sortable style={{ minWidth: "8rem" }} />
<Column field="name" header="Name" body={pTemplate1} filter={selectedFilterFields.includes("name")} hidden={selectedHideFields?.includes("name")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload CodeEditor Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search CodeEditor" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default CodeEditorDataTable;
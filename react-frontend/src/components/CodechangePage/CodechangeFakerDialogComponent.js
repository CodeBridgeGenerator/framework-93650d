...
import codechangeFakerFactory from "./codechangeFakerFactory";
...
const CodechangeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = codechangeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("codechange").create(elem));
...
export default connect(mapState, mapDispatch)(CodechangeFakerDialogComponent);

...
const CodechangeDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.change}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "codechange"});
...
<Column field="location" header="Location" body={pTemplate0} filter={selectedFilterFields.includes("location")} hidden={selectedHideFields?.includes("location")}  sortable style={{ minWidth: "8rem" }} />
<Column field="filename" header="Filename" body={pTemplate1} filter={selectedFilterFields.includes("filename")} hidden={selectedHideFields?.includes("filename")}  sortable style={{ minWidth: "8rem" }} />
<Column field="change" header="Change" body={pTemplate2} filter={selectedFilterFields.includes("change")} hidden={selectedHideFields?.includes("change")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Codechange Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Codechange" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default CodechangeDataTable;
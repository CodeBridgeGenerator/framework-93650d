...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.env?.appName}</p>
const dropdownArrayTemplate1 = (rowData, { rowIndex }) => <p >{rowData.type}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "environments"});
...
<Column field="env" header="Env" body={dropdownTemplate0} filter={selectedFilterFields.includes("env")} hidden={selectedHideFields?.includes("env")}  style={{ minWidth: "8rem" }} />

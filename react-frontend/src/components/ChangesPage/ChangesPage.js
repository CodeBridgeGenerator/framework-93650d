...
import ChangesFakerDialogComponent from "./ChangesFakerDialogComponent";
import ChangesSeederDialogComponent from "./ChangesSeederDialogComponent";
...
const ChangesPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("changes")
            .find({ query: { $limit: 10000 , change : urlParams.singleCodechangesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                console.log({ error });
                setLoading(false);
                props.alert({ title: "Changes", type: "error", message: error.message || "Failed get Changes" });
            });

    const service = _.find(config.services, { serviceName : "changes"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("changes").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Changes", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("changes").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Changes </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="changes-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="changes-datatable">
                    <ChangesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <ChangesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ChangesCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <ChangesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ChangesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ChangesPage);

...
import codeEditorFakerFactory from "./codeEditorFakerFactory";
...
const CodeEditorFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = codeEditorFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("codeEditor").create(elem));
...
export default connect(mapState, mapDispatch)(CodeEditorFakerDialogComponent);

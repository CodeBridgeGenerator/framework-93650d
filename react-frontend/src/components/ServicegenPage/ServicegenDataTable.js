...
const ServicegenDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownArrayTemplate0 = (rowData, { rowIndex }) => <p >{rowData.service}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "servicegen"});
...
<Column field="service" header="Service" body={dropdownArrayTemplate0} filter={selectedFilterFields.includes("service")} hidden={selectedHideFields?.includes("service")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Servicegen Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Servicegen" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ServicegenDataTable;
...
import CodechangeDatatable from "./CodechangeDataTable";
import CodechangeEditDialogComponent from "./CodechangeEditDialogComponent";
import CodechangeCreateDialogComponent from "./CodechangeCreateDialogComponent";
import CodechangeFakerDialogComponent from "./CodechangeFakerDialogComponent";
import CodechangeSeederDialogComponent from "./CodechangeSeederDialogComponent";

const CodechangePage = (props) => {
    ...
    const service = _.find(config.services, { serviceName : "codechange"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("codechange").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Codechange", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("codechange").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Codechange </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="codechange-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="codechange-datatable">
                    <CodechangeDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <CodechangeEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CodechangeCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <CodechangeFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CodechangeSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(CodechangePage);

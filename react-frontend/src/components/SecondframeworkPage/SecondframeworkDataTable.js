...
const SecondframeworkDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const Port = JSON.parse('{"1":"A","2":"B","3":"C","4":"D"}');
const dropdownObjectTemplate0 = (rowData, { rowIndex }) => <p >{Port[rowData.Port]}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "secondframework"});
...
<Column field="Port" header="Port" body={dropdownObjectTemplate0} filter={selectedFilterFields.includes("Port")} hidden={selectedHideFields?.includes("Port")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Secondframework Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Secondframework" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default SecondframeworkDataTable;
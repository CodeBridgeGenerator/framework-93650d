...
import servicegenFakerFactory from "./servicegenFakerFactory";
...
const ServicegenFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = servicegenFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("servicegen").create(elem));
...
export default connect(mapState, mapDispatch)(ServicegenFakerDialogComponent);

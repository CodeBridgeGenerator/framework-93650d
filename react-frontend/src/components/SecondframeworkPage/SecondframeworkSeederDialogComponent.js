...
import seederData from "./secondframework.seeder.json";
...
const SecondframeworkSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("secondframework").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(SecondframeworkSeederDialogComponent);

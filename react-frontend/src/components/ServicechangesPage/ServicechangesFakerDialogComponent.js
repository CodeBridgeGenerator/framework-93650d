...
import servicechangesFakerFactory from "./servicechangesFakerFactory";
...
const ServicechangesFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("servicegentemp")
                        .find({ query: { $select: ["_id","templateLocalFile"] } })
                        .then((data) => {
                        setChangeIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = servicechangesFakerFactory(props.user,fakerCount,ChangeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("servicechanges").create(elem));
...
export default connect(mapState, mapDispatch)(ServicechangesFakerDialogComponent);

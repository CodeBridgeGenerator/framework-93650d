...
import ProtocolCreateDialogComponent from "./ProtocolCreateDialogComponent";
import ProtocolFakerDialogComponent from "./ProtocolFakerDialogComponent";
import ProtocolSeederDialogComponent from "./ProtocolSeederDialogComponent";
...
const ProtocolPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("protocol")
            .find({ query: { $limit: 10000 , protocol : urlParams.singleSettingsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
    ...
    const service = _.find(config.services, { serviceName : "protocol"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("protocol").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Protocol", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("protocol").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Protocol </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="protocol-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="protocol-datatable">
                    <ProtocolDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <ProtocolEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ProtocolCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <ProtocolFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ProtocolSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ProtocolPage);

...
const ServicechangesDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.Change?.templateLocalFile}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.typeofchanges}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "servicechanges"});
...
<Column field="typeofchanges" header="Typeofchanges" body={pTemplate1} filter={selectedFilterFields.includes("typeofchanges")} hidden={selectedHideFields?.includes("typeofchanges")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Servicechanges Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Servicechanges" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ServicechangesDataTable;
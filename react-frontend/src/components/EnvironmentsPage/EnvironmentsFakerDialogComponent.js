...
import environmentsFakerFactory from "./environmentsFakerFactory";
...
const EnvironmentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = environmentsFakerFactory(props.user,fakerCount,envIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("environments").create(elem));
...
export default connect(mapState, mapDispatch)(EnvironmentsFakerDialogComponent);

...
import codechangesFakerFactory from "./codechangesFakerFactory";
...
const CodechangesFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("appgentemp")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        seteditorIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = codechangesFakerFactory(props.user,fakerCount,editorIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("codechanges").create(elem));
...
export default connect(mapState, mapDispatch)(CodechangesFakerDialogComponent);

...
import secondframeworkFakerFactory from "./secondframeworkFakerFactory";
...
const SecondframeworkFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = secondframeworkFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("secondframework").create(elem));
...
export default connect(mapState, mapDispatch)(SecondframeworkFakerDialogComponent);

...
import changesFakerFactory from "./changesFakerFactory";
...
const ChangesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = changesFakerFactory(props.user,fakerCount,changeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("changes").create(elem));
...
export default connect(mapState, mapDispatch)(ChangesFakerDialogComponent);
